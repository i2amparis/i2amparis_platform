$(document).ready(function () {
    ...
    $("#energy_co2-clear-button").click(function () {
        $('#energy_co2 select.sum-boot-select').multipleSelect('setSelects', []);
        $('#energy_co2_viz_frame_div').hide();
    });

    $("#energy_co2-run-button").click(function () {
        var viz_frame = $('#energy_co2_viz_frame_div');
        var model_sel = $('#energy_co2_model_name');
        var scenario_sel = $('#energy_co2_scenario_name');
        var region_sel = $('#energy_co2_region_name');
        ...
        if (model_full || scenario_full || region_full) {
            ...
            /* # Query creation*/
            var jq_obj = create_energy_co2_query();
            console.log('Energy JSON Query Created');
            retrieve_series_model_scenario(jq_obj);

        }
    });

    function start_qc_v_energy_co2_process(variable, json_query_obj){
        var query = {};
        query["query_name"] = "energy_co2_query";
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log('Energy Query Saved in DB');
                var query_id = data['query_id'];
                create_visualisation_energy_co2(query_id, json_query_obj, variable);
            },
            error: function (data) {
                console.log(data);
            }
    ...
    function create_visualisation_energy_co2(query_id, json_query_obj, variable) {
        ...
        var viz_frame = $('#energy_co2_viz_iframe');
        viz_frame.off();
        viz_frame.hide();
        $('#energy_co2_loading_bar').show();
        ...
        console.log('Energy Ready to launch visualisation');
        var complete_url = "/visualiser/show_line_chart?" + url;
        viz_frame.attr('src', complete_url);
        viz_frame.on('load', function () {
            console.log('Energy Visualisation Completed');
            $(this).show();
            $.ajax({
                ...
                contentType: 'application/json',
                success: function (data) {
                    console.log("Energy Temporary Query Deleted");
            ...
            $('#energy_co2_loading_bar').hide();

        });

    }

    function create_energy_co2_query() {
        var sel_model = $('#energy_co2_model_name');
        var sel_scenario = $('#energy_co2_scenario_name');
        var sel_region = $('#energy_co2_region_name');
    ...
    function retrieve_series_model_scenario(jq_obj) {
        ...
        $.ajax({
            ...
            contentType: 'application/json',
            success: function (data) {
                console.log('Energy Unit Info Retrieved');
                ...
                start_qc_v_energy_co2_process(jq_obj["variables"], json_object)
    ...
    $("#energy_co2-run-button").trigger('click');
});
