$(document).ready(function () {
    ...
    $("#industry_co2-clear-button").click(function () {
        $('#industry_co2 select.sum-boot-select').multipleSelect('setSelects', []);
        $('#industry_co2_viz_frame_div').hide();
    });

    $("#industry_co2-run-button").click(function () {
        var viz_frame = $('#industry_co2_viz_frame_div');
        var model_sel = $('#industry_co2_model_name');
        var scenario_sel = $('#industry_co2_scenario_name');
        var region_sel = $('#industry_co2_region_name');
        ...
        if (model_full || scenario_full || region_full) {
            ...
            /* # Query creation*/
            var jq_obj = create_industry_co2_query();
            console.log('Industry JSON Query Created');
            retrieve_series_model_scenario(jq_obj);

        }
    });

    function start_qc_v_industry_co2_process(variable, json_query_obj){
        var query = {};
        query["query_name"] = "industry_co2_query";
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log('Industry Query Saved in DB');
                var query_id = data['query_id'];
                create_visualisation_industry_co2(query_id, json_query_obj, variable);
            },
            error: function (data) {
                console.log(data);
            }
    ...
    function create_visualisation_industry_co2(query_id, json_query_obj, variable) {
        ...
        var viz_frame = $('#industry_co2_viz_iframe');
        viz_frame.off();
        viz_frame.hide();
        $('#industry_co2_loading_bar').show();
        ...
        console.log('Industry Ready to launch visualisation');
        var complete_url = "/visualiser/show_line_chart?" + url;
        viz_frame.attr('src', complete_url);
        viz_frame.on('load', function () {
            console.log('Industry Visualisation Completed');
            $(this).show();
            $.ajax({
                ...
                contentType: 'application/json',
                success: function (data) {
                    console.log("Industry Temporary Query Deleted");
            ...
            $('#industry_co2_loading_bar').hide();

        });

    }

    function create_industry_co2_query() {
        var sel_model = $('#industry_co2_model_name');
        var sel_scenario = $('#industry_co2_scenario_name');
        var sel_region = $('#industry_co2_region_name');
        var variable = ['Emissions|CO2|Industrial Processes'];
    ...
    function retrieve_series_model_scenario(jq_obj) {
        ...
        $.ajax({
            ...
            contentType: 'application/json',
            success: function (data) {
                console.log('Industry Unit Info Retrieved');
                ...
                start_qc_v_industry_co2_process(jq_obj["variables"], json_object)
    ...
    $("#industry_co2-run-button").trigger('click');
});
